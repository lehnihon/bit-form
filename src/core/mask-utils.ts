...
export const createCurrencyMask = ({ symbol, thousand, decimal, precision = 2 }: CurrencyConfig) => {
  return (value: number | string | null | undefined) => {
    if (value === undefined || value === null || value === '') return '';

    let stringValue = typeof value === 'number' 
...
export const maskBRL = createCurrencyMask({ symbol: 'R$', thousand: '.', decimal: ',' });
export const maskUSD = createCurrencyMask({ symbol: '$', thousand: ',', decimal: '.' });