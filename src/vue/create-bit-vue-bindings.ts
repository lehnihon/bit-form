...
export interface BitVueBindings<TForm extends object = any> {
...
const bindingCache = new WeakMap<object, BitVueBindings<any>>();

function buildBindings<TForm extends object>(
  store: BitFrameworkStoreApi<TForm>,
): BitVueBindings<TForm> {
  return {
...
export function createBitVueBindings<TForm extends object>(
...
export function createBitVueForm<
  TForm extends object = Record<string, unknown>,
>(config: BitConfig<TForm> = {}): BitVueBindings<TForm> {
  const store = createBitStore<TForm>(config);
  return createBitVueBindings(store);
}
